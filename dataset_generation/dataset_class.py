...
class SyntheticDataset(Dataset):
    ...
    def __getitem__(self, index):
        raw_img_path = os.path.join(self.raw_dir, str(index))
        raw_image = decode_image(raw_img_path)
        label_image_path = os.path.join(self.label_dir)
        label_image = decode_image(label_image_path)
        if self.transform:
